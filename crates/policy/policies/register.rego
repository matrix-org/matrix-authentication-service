...
violation[{"field": "password", "msg": msg}] {
	...
	msg := sprintf("needs to be at least %d characters", [input.user.password])
}

violation[{"field": "password", "msg": "requires at least a number"}] {
	data.passwords.require_number
	not regex.match("[0-9]", input.user.password)
}

violation[{"field": "password", "msg": "requires at least a lowercase letter"}] {
	data.passwords.require_lowercase
	not regex.match("[a-z]", input.user.password)
}

violation[{"field": "password", "msg": "requires at least an uppercase letter"}] {
